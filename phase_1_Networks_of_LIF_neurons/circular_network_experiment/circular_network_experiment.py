...
# Call the standalone visualization function
    ...
    # Optimized neuron selection for large networks
    print("Identifying neurons of interest for tracking...")

    # If no stimulation was applied, select neurons based on activity
    if no_stimulation:
        # Use numpy array for counting
        active_count = np.zeros(n_neurons, dtype=int)
        for active_indices in activity_record:
            if len(active_indices) > 0:
                active_count[active_indices] += 1

        # Get indices of top 3 most active neurons
        stimulated_neurons = np.argsort(active_count)[-3:].tolist()
        print(f"Most active neurons: {stimulated_neurons} (activity counts: {active_count[stimulated_neurons]})")
    else:
        # Use numpy array for counting stimulations
        stim_count = np.zeros(n_neurons, dtype=int)
        for neurons_list in stimulation_record['neurons']:
            if len(neurons_list) > 0:
                stim_count[neurons_list] += 1

        # Get indices of top 3 most stimulated neurons
        stimulated_neurons = np.argsort(stim_count)[-3:].tolist()
        print(f"Most stimulated neurons: {stimulated_neurons} (stim counts: {stim_count[stimulated_neurons]})")

    # Find neurons connected to these stimulated/active ones using vectorized operations
    print("Finding connected neurons...")
    connected_neurons = []
    for src in stimulated_neurons:
        # Get all targets from this source with non-zero weights
        targets = np.where(network.weights[src, :] != 0)[0]
        # Filter out already selected neurons
        valid_targets = [tgt for tgt in targets if tgt not in stimulated_neurons and tgt not in connected_neurons]
        # Add up to 1 connected neuron per source (total of 3)
        if valid_targets:
            connected_neurons.append(valid_targets[0])
        if len(connected_neurons) >= 3:
            break
    ...
    if no_stimulation:
        ...
        # Get list of all neurons ever stimulated more efficiently
        all_stimulated_set = set()
        for neuron_list in stimulation_record['neurons']:
            all_stimulated_set.update(neuron_list)
        all_stimulated_neurons = list(all_stimulated_set)
    ...
    # Clean up memory from first simulation before starting second run
    print("Cleaning up memory from first simulation...")
    # Delete large data structures we no longer need
    del activity_record
    del stimulation_record
    print("Memory cleanup complete.")
    print("Starting second simulation run...")
