...
def _add_output_mutual_inhibition(network, layer_indices, positions, connection_map,
    ...
    """
    Add learnable mutual inhibition between all output layer neurons.

    This creates winner-take-all dynamics where output neurons compete.
    These connections ARE evolved by the GA - they start with weight=0
    and will be optimized along with other connections.

    Note: The inhibitory_weight parameter is now unused (kept for backwards compatibility).
    """
    ...
    print(f"  Adding learnable mutual inhibition between output neurons {output_start}-{output_end-1}")
    ...
    for i in range(output_start, output_end):
        ...
        for j in range(output_start, output_end):
            ...
            # Add connection with weight=0 (to be evolved by GA)
            network.add_connection(i, j, weight=0.0, delay=delay)
    ...
    print(f"  Mutual inhibition connections (learnable): {manual_connections}")
