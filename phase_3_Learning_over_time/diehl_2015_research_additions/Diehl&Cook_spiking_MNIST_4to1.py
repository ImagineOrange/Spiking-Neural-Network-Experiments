...
def normalize_weights():
    """Normalize weights so each excitatory neuron receives constant total input.

    Uses sparse matrix operations to avoid dense 784x400 allocations.
    """
    for connName in connections:
        if connName[1] == 'e' and connName[3] == 'e':
            conn = connections[connName]
            # Build sparse matrix directly from connection data
            sp_matrix = sparse.csr_matrix(
                (conn.w[:], (conn.i[:], conn.j[:])),
                shape=(n_input, n_e)
            )
            # Compute column sums efficiently (sparse sum)
            col_sums = np.array(sp_matrix.sum(axis=0)).flatten()
            # Compute scaling factors
            col_factors = weight['ee_input'] / col_sums
            # Scale each weight by its column's factor (conn.j gives column index)
            conn.w[:] = conn.w[:] * col_factors[conn.j[:]]
...
noise_sigma_e = cfg.noise_sigma_e
noise_sigma_i = cfg.noise_sigma_i
...
for name in input_connection_names:
    ...
    for connType in input_conn_names:
        ...
        # ei_input (Input→Inhibitory) connections are fixed and always loaded from random/
        # ee_input (Input→Excitatory) connections are learned and loaded from weight_path
        ...
        try:
            ...
            print(f"Missing file: {load_path}{connName}{ending}.npy")
            print(f"\nPlease run 'Diehl&Cook_MNIST_random_conn_generator.py' first to generate initial weights.")
            exit(1)
        ...
        # 4:1 FIX: STDP only applies to ee_input (Input→Excitatory), not ei_input (Input→Inhibitory)
        # ei_input connections are fixed weights and must use the non-STDP branch
        use_stdp_for_this_conn = ee_STDP_on and connType == 'ee_input'

        if not use_stdp_for_this_conn:
            # Non-STDP connections (test mode, or Input→Inhibitory in train mode)
            ...
            # STDP connections (train mode, Input→Excitatory only)
            stdp_methods[connName] = Synapses(src_grp, tgt_grp, model=eqs_stdp_ee,
                                              on_pre=eqs_stdp_pre_ee, on_post=eqs_stdp_post_ee,
                                              delay=delay[connType][1], method='euler')
            sources, targets = np.where(weightMatrix > 0)
            stdp_methods[connName].connect(i=sources, j=targets)
            stdp_methods[connName].w = weightMatrix[sources, targets]
            stdp_methods[connName].delay = 'rand() * ' + str(float(delay[connType][1]/ms)) + '*ms'
            connections[connName] = stdp_methods[connName]
...
# Sliding window for tracking neurons fired (for monitoring specialization)
neurons_fired_window = []
neurons_fired_window_size = 100
...
for epoch_images, epoch_labels, epoch_num in epoch_generator:
    ...
    for example_idx in range(len(epoch_labels)):
        ...
        # Track neurons fired in sliding window
        neurons_fired = np.sum(current_spike_count > 0)
        neurons_fired_window.append(neurons_fired)
        if len(neurons_fired_window) > neurons_fired_window_size:
            neurons_fired_window.pop(0)

        # Periodic weight and spike statistics
        if j % 100 == 0 and j > 0:
            if 'XeAe' in connections:
                weights = connections['XeAe'].w[:]
                # Calculate sliding window average
                avg_neurons_fired = np.mean(neurons_fired_window)
                pbar.write(f'[{j:5d}] Weight stats: avg={np.mean(weights):.4f}, '
                          ...
                          f'E fired (100-avg): {avg_neurons_fired:.1f}/400 ({avg_neurons_fired/4:.1f}%)')
