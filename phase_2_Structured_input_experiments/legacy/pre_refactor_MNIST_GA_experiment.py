...
def Layered_run_unified_GA_simulation(network: LayeredNeuronalNetworkVectorized, # Type hint updated
...
def run_snn_simulation(network: LayeredNeuronalNetworkVectorized, # Type hint updated
    ...
    activity_record, _, _ = Layered_run_unified_GA_simulation(
        network,
        duration=duration,
        dt=dt,
...
if __name__ == "__main__":
    ...
    ENCODING_MODE = 'intensity_to_neuron' # Options: 'intensity_to_neuron' or 'conv_feature_to_neuron'
    ...
    layers_config_global = [input_neurons, 40, 30,  N_CLASSES] # Uses the correct input_neurons now

    inhib_frac_global = 0.2
    conn_probs_global = { # Keep using probabilities for initial structure generation
        'exc_recurrent': 0.0, 'inh_recurrent': 0.30, 'feedforward_1': 0.3,
